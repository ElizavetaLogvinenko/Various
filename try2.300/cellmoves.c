...
// cellular potts model: one Monte Carlo step
{
	...
	for(i=0;i<NRsteps;i++)
	{
		...
		//if((xtx>0)&&(xtx<NVX*0.3)&&(xty>0)&&(xty<NVY*0.3)&&(xty>NVY*0.8)&&(xty<NVY-1)&&(xtx>NVX*0.8)&&(xtx<NVX-1)) // exclude outer rim
		{
			...
			if(go_on)
			{
        		...
        		//modification 15.01.22
			if(((abs(xtx-NVX/2)<NVX/40&&xty<=NVY/2)||(abs(xty-NVY/2)<NVY/40&&xtx>=NVX/2))||((abs(xtx-NVX/2)<NVX/40&&xsy<=NVY/2)||(abs(xty-NVY/2)<NVY/40&&xsx>=NVX/2))){
			pdms_penalti=0;
			//printf("i am here!");
			dH = calcdH(pv,pf,CMs,csize,xt,xs,pick,ttag,stag)+pdms_penalti;//pdms_penalti is energy penalti for cells near the pdms
			//printf("dH %.4f", dH);
			}
			//modification 15.01.22 ends	
...
// cellular potts model: one Monte Carlo step
{
	...
	for(i=0;i<NRsteps;i++)
	{
		...
		if((xtx>0)&&(xtx<NVX-1)&&(xty>0)&&(xty<NVY-1)) // exclude outer rim
		{
			...
			pdms_penalti=0;
			//printf("i am here!");
			dH = calcdH_CH(pv, CMs, xt, xs)+pdms_penalti;//pdms_penalti is energy penalti for cells near the pdms
			//printf("dH %.4f", dH);
			}
			//modification 15.01.22 ends			
