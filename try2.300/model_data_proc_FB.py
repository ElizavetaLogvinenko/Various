...
#     image_label_overlay = label2rgb(label_image, image=image, bg_label=0)
    ...
    if path == None:
        plt.savefig('approx_FB_{}.jpeg'.format(iter_num))
        uni_npy = np.array([param_arr, res_arr, sigma_arr])
        np.save('current_par_FB_{}.npy'.format(iter_num), uni_npy)
    else:
        plt.savefig(str(path)+'approx_FB_{}.jpeg'.format(iter_num))
        uni_npy = np.array([param_arr, res_arr, sigma_arr])
        npy_path=str(path)+'current_par_FB_{}.npy'.format(iter_num)
